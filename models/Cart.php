...
class Cart {
    ...
    public function addToCart($product, $quantity) {
        if (isset($this->cart[$product->get_idProduit()])) {
            $this->cart[$product->get_idProduit()]['quantity'] += $quantity;
        } else {
            $this->cart[$product->get_idProduit()] = ['product' => $product, 'quantity' => $quantity];
        }
        ...
        $output .= "<article class='d-flex flex-column justify-content-between align-items-center min-vh-100'>";
            $output .= "<ul class='list-group list-group-flush'>";
                foreach($this->cart as $id => $product) {
                    $p = $product['product'];
                    $p->set_quantity($product['quantity']);
                    $this->total += $p->get_prix() * $p->get_quantity();
                    $d = $p->get_data();
                    $card = new Card($d);
